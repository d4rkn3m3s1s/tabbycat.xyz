...
let kaomTexts = document.querySelectorAll('.kaom__text span');

function randomTranslate(element) {
    setInterval(function() {
        // Генерируем случайные значения для смещения по оси X и Y от -30 до 30
        // var translateX = ; // От -30 до 30
        // var translateY = ; // От -30 до 30
        
        // Применяем случайные значения смещения к элементу
		for (let index = 0; index < kaomTexts.length; index++) {
	const element = kaomTexts[index];
	 element.style.transform = `translate(${Math.floor(Math.random() * 61) - 30}px, ${Math.floor(Math.random() * 61) - 30}px) scale(${Math.random() * (1.4 - 0.7) + 0.7})`;
}
       
    }, 5000); // Выполняем каждые 5 секунд (5000 миллисекунд)
}
randomTranslate();

let kaomYeys = document.querySelector('.kaom__yeys');
let loaderVideo = document.querySelector('.loader video');

loaderVideo.style.left = kaomYeys.getBoundingClientRect().left+'px'
loaderVideo.style.top = kaomYeys.getBoundingClientRect().top+'px'
loaderVideo.style.width = kaomYeys.clientWidth+'px'
